...
void Map::Build() {
	for (int y = 0; y < this->height; y++) {
		for (int x = 0; x < this->width; x++) {
			...
			float tileHeight = (31.0f/32.0f) / (float)tile_count_y;
			...
			vertices.insert(vertices.end(), {
				...
				yoffset + -tile_size * y,
				xoffset + (tile_size * x),
				yoffset + (-tile_size * y) - tile_size,
				xoffset + (tile_size * x) + tile_size,
				yoffset + (-tile_size * y) - tile_size,
				xoffset + (tile_size * x),
				yoffset + -tile_size * y,
				xoffset + (tile_size * x) + tile_size,
...
//In this game, the enemy should not be able to go 

bool Map::IsSolidPlayer(glm::vec3 position, float *penetration_x, float *penetration_y) {
	*penetration_x = 0;  
	*penetration_y = 0;     
	...
	if (tile == 0 || tile >= 9 || tile == 6) return false;     
...
bool Map::IsSolid(glm::vec3 position, float *penetration_x, float *penetration_y) { //for enemies 
	*penetration_x = 0;
	*penetration_y = 0;
	if (position.x < left_bound || position.x > right_bound) return false;
	if (position.y > top_bound || position.y < bottom_bound) return false;
